...
void ScenePlugin::build(game::Game &game) {
  ...
  game.world.addSystem<
      ResMut<Commands>, ResMut<IRenderer *>,
    ...
    view.forEach([](auto view, Entity e, Commands &cmd, IRenderer *renderer,
      ...
      const std::string &file_path = scene_name.load_scene;
...
#pragma endregion
          ...
          if (!value.empty()) {
            ...
            if (it == loaded_images.end()) {
              ...
              std::vector<uint8_t> img_data =
                  loadBinaryFile(file_path + "images/" + id + ".img_data.bin");
      ...
      for (auto &e : entities) {
        ...
        Mat4<float> transform_matrix = Mat4<float>::createTransformMatrix(
            transform.translation, transform.scale, e.rotation);

        GpuTransform gpu_transform{};

        auto transform_handle = render_buffers.data.at(BufferType::Transform);

        gpu_transform.transform_handle = renderer->writeBuffer(
            transform_handle, &transform_matrix, sizeof(transform_matrix),
            UINT32_MAX, VK_ACCESS_SHADER_READ_BIT);
        ...
        for (auto &m : e.meshes) {
          ...
          cmd.push([transform, gpu_transform, name, mesh_renderer](Ecs *world) {
