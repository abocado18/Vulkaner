...
render::RenderContext::RenderContext(uint32_t width, uint32_t height)
{
...

    {
        ...
        for (const VkPhysicalDevice &d : physical_devices)
        {
            ...
            auto areExtensionsSupported = [&](const std::vector<std::string> &required_extensions) -> bool
            {
                ...
                vkEnumerateDeviceExtensionProperties(d, nullptr, &extension_count, extension_properties.data());
...

    {
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(this->physical_device, &count, queue_family_properties.data());
...

    {
        ...
        VK_ERROR(vkCreateDevice(this->physical_device, &create_info, nullptr, &this->device));
...

    {
        ...
        VK_ERROR(vkAllocateCommandBuffers(device, &allocate_info, &primary_command_buffer));
    }

    {
        VkSemaphoreCreateInfo create_info = {};
        create_info.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
        VK_ERROR(vkCreateSemaphore(device, &create_info, nullptr, &aquire_image_semaphore));

        rendering_finished_semaphores.resize(swapchain.images.size());

        for (size_t i = 0; i < swapchain.images.size(); i++)
        {
            VK_ERROR(vkCreateSemaphore(device, &create_info, nullptr, &rendering_finished_semaphores[i]));
        }
    }
...
void render::RenderContext::render()
{
    ...
    VkResult aquire_image_result = vkAcquireNextImage2KHR(device, &aquire_info, &swapchain_image_index);

    if (aquire_image_result == VK_ERROR_OUT_OF_DATE_KHR || aquire_image_result == VK_SUBOPTIMAL_KHR)
    {
        vkDeviceWaitIdle(device);
...

    {

        {
            ...
            barrier.image = swapchain.images[swapchain_image_index];
            ...
            vkCmdPipelineBarrier2KHR(primary_command_buffer, &depInfo);
        }
...

    {
        ...
        memory_barrier.srcStageMask = VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR;
        memory_barrier.srcAccessMask = VK_ACCESS_2_COLOR_ATTACHMENT_READ_BIT_KHR;
        memory_barrier.dstAccessMask = VK_ACCESS_2_MEMORY_READ_BIT_KHR;
        memory_barrier.dstStageMask = VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR;
        memory_barrier.image = swapchain.images[swapchain_image_index];
        ...
        vkCmdPipelineBarrier2KHR(primary_command_buffer, &dependency_info);
    }
    ...
    wait_submit_info.stageMask = VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR;
    ...
    signal_submit_info.semaphore = rendering_finished_semaphores[swapchain_image_index];
    ...
    present.pWaitSemaphores = &rendering_finished_semaphores[swapchain_image_index];
...
void render::RenderContext::createSwapchain(bool has_old_swapchain, Swapchain old_swapchain)
{
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(this->physical_device, this->surface, &surface_format_count, surface_formats.data());
    ...
    VK_ERROR(vkCreateSwapchainKHR(this->device, &create_info, nullptr, &this->swapchain.swapchain));
    ...
    vkGetSwapchainImagesKHR(device, swapchain.swapchain, &swapchain_image_count, swapchain.images.data());
    ...
    for (size_t i = 0; i < swapchain_image_count; i++)
    {
        ...
        VK_ERROR(vkCreateImageView(this->device, &view_create_info, nullptr, &this->swapchain.views[i]));
    }
